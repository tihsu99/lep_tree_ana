...
class ResponseMatricesManager:
    ...
    def closure_test(self, region, signal_name, var):
        ...
        h_truth = unfold.build_TH1D(f"h_truth_{region}_{signal_name}_{var}", var_truth_binned, num_bins=self.num_bins, weight=weight[mask_truth_region])
        ...
        h_recon = unfold.build_TH1D(f"h_recon_{region}_{signal_name}_{var}", var_recon_binned, num_bins=self.num_bins, weight=weight[mask_analysis_region])

        # unfold reco distribution and compare with truth distribution
        unfold_result = ROOT.RooUnfoldSvd(response_matrix, h_recon, 5).Hunfold(2)
        # roounfold = ROOT.RooUnfoldInvert(response_matrix, h_recon)
        # unfold_result = roounfold.Hunfold()
        ...
        c.BuildLegend()
        c.SaveAs(f"{self.output_dir}/closure_{region}_{signal_name}_{var}.png")
