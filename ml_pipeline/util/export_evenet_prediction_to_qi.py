...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional fraction represented by the EveNet-predicted subset, e.g. 0.5 for a test split. "
            "Only rows with EveNet prediction get central weight scaled by 1/fraction; all default/raw-only "
            "rows keep their original central weight."
        ),
    )
    parser.add_argument(
        "--write-baseline-copy",
...
def with_evenet_reconstruction(
    ...
    if "weight" in output.fields:
        ...
        if prediction_split_fraction is not None:
        ...
        output["weight"] = (original_weight * weight_scale.astype(np.float64)).astype(np.float32)
    ...
    metrics["prediction_weight_scale"] = float(1.0 / prediction_split_fraction) if prediction_split_fraction else 1.0
...
def choose_event_key(full_events: ak.Array, pred_source_events: ak.Array) -> str | None:
    for key in ("evtNumber", "Event_evtNumber"):
        if key in full_events.fields and key in pred_source_events.fields:
            return key
    return None
...
def map_prediction_rows_to_full(
    ...
    key = choose_event_key(full_events, pred_source_events)
    if key is not None:
        full_keys = ak.to_numpy(full_events[key], allow_missing=False)
        pred_keys = ak.to_numpy(pred_source_events[key], allow_missing=False)
        full_lookup: dict[Any, int] = {}
        duplicate_keys: set[Any] = set()
        for index, value in enumerate(full_keys.tolist()):
            if value in full_lookup:
                duplicate_keys.add(value)
            else:
                full_lookup[value] = index
        if duplicate_keys:
            raise ValueError(
                f"Cannot merge by {key}: full central parquet has duplicate event keys. "
                "Use a row-aligned prediction-central parquet with the same length as full raw, "
                "or add a unique event id before conversion."
            )
        missing = [value for value in pred_keys.tolist() if value not in full_lookup]
        if missing:
            preview = missing[:5]
            raise ValueError(
                f"Cannot merge predictions: {len(missing)} prediction-central events are not in full raw "
                f"when matching by {key}. First missing keys: {preview}"
            )
        return np.asarray([full_lookup[value] for value in pred_keys.tolist()], dtype=np.int64)
...
def export_config_prediction(
    ...
    for source_index in sorted(np.unique(source_indices).tolist()):
        ...
        lookup = {value: idx for idx, value in enumerate(selected_keys.tolist())}
        missing = [value for value in pred_keys.tolist() if value not in lookup]
        if missing:
            raise ValueError(
                f"{pred_path} has {len(missing)} events not present in analysis input_files for source "
                f"{source_info['expanded_name']}. First missing keys: {missing[:5]}"
            )
        selected_indices = np.asarray([lookup[value] for value in pred_keys.tolist()], dtype=np.int64)
    ...
    for parent_name, group in grouped.items():
        ...
        evenet_events, metrics = merge_prediction_group(
            full_events=full_events,
