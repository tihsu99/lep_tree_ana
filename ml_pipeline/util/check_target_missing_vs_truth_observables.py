...
def build_tau_tau_pair(tau_a: ak.Array, tau_b: ak.Array, visible_a: ak.Array, visible_b: ak.Array) -> ak.Array:
    energy = CM_ENERGY / 2
    mass = TAU_MASS
...
def predicted_reconstructed_values(events: ak.Array, observable: str) -> np.ndarray:
    ...
    #
    # tau_a, tau_b = build_tau_tau_pair(visible_a + pred_a, visible_b + pred_b, visible_a, visible_b)
    #
...
def reconstructed_chain_values(events: ak.Array, missing_kind: str) -> dict[str, np.ndarray]:
    ...
    # tau_a, tau_b = build_tau_tau_pair(visible_a + missing_a, visible_b + missing_b, visible_a, visible_b)
    ...
    values: dict[str, np.ndarray] = {
        ...
        f"{prefix}_tautau_sum_pt": np.asarray(tau_a.pt + tau_b.pt, dtype=np.float64),
        f"{prefix}_tautau_sum_phi": np.asarray(tau_b.phi + tau_b.phi, dtype=np.float64),
        f"{prefix}_tautau_sum_eta": np.asarray(tau_a.eta + tau_b.eta, dtype=np.float64),
    }
    for axis in ("n", "r", "k"):
